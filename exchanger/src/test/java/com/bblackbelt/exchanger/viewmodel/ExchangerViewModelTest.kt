...
class ExchangerViewModelTest {
    ...
    fun `data loaded success`() {
        ...
        val exchangeRates = listOf(Rate("USD", 0.89f), Rate("DKK", 1.2324f))
        ...
        val v = viewModel.rates.getValueTimeout()

        Assert.assertEquals(v, mapper.map(exchangeRates, 0f))
    }

    @Test
    fun `test data load fail`() {
        val baseRate = "EUR"
        Mockito.`when`(useCase.loadRates(baseRate)).thenReturn(Single.error(Throwable()))

        val viewModel = ExchangerViewModel(useCase, mapper)
        viewModel.updateBaseRate(RateView(baseRate, 1f, ""))

        Assert.assertEquals(viewModel.rates.getValueTimeout(), listOf<RateView>())
    }
    ...
    fun `exchanged value updated`() {
        ...
        Assert.assertEquals( viewModel.rates.getValueTimeout() , mapper.map(exchangeRates, valueToConvert))
    }

    @Test
    fun `exchanged value updated after data load fail`() {
        val baseRate = "EUR"
        Mockito.`when`(useCase.loadRates(baseRate)).thenReturn(Single.error(Throwable()))
